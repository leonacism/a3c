...
abstract Vec3(Vec3_) from Vec3_ to Vec3_ {
	...
	@:op(A + B)
	static public inline function add(v1:Vec3, v2:Vec3):Float return new Vec3(v1.x + v2.x, v1.y + v2.y, v1.z + v2.z);

	
	@:op(A - B)
	static public inline function sub(v1:Vec3, v2:Vec3):Float return new Vec3(v1.x - v2.x, v1.y - v2.y, v1.z - v2.z);
	
	@:op(A * B)
	@:commutative
	static public inline function scale(v:Vec3, s:Float):Float return new Vec3(v.x*s, v.y*s, v.z*s);
	
	@:op(A / B)
	static public inline function div(v:Vec3, s:Float):Float return new Vec3(v.x/s, v.y/s, v.z/s);
